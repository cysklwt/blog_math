...
1. 当 $f(x) = P_{n}(x) e^{ax}$ 时，特解需要设为： $$y^* = e^{ax}Q_{n}(x) x^{k}$$
特解设为 $y^*=e^{\alpha x}Q_n(x)x^k$，其中 $e^{\alpha x}$ 照抄，$Q_n(x)$ 为 $x$ 的 $n$ 次多项式，
$$
k=
\begin{cases}  
0，& \alpha \text{不是特征根} \\
...
$$
 ...
 1. 当 $f(x) = e^{ax}[P_{m}(x)\cos \beta x +P_{n}(x)\sin \beta x]x^{k}$ 时，特解需要设为： $$y^* = e^{ax}[Q_{l}^{(1)}(x)\cos \beta x + Q_{l}^{(2)}(x) \sin \beta x] x^{k}$$
特解设为 $y^*=e^{\alpha x}Q_n(x)x^k$，其中$e^{\alpha x}$照抄，$l=\max\{m,n\}$，$Q_l^{(1)}$、$Q_l^{(2)}$为$x$的两个不同的$l$次多项式，
$$
k=
\begin{cases}
