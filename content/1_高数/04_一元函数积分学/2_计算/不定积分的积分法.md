...
当被积函数分母的幂次比分子**高 2 次及以上**时，令 $x=\frac{1}{t}$。
...
> > ![分部积分](https://animed-blog.oss-cn-wuhan-lr.aliyuncs.com/fenbu_exp.png) 
...
> 此时可以使用 **展开看系数列方程** 求解，或者 **代特殊 x 值消元（推荐）** 求解。解得：
> $$A = 1, B = 0, C = -2$$
> $$ 原式 = \int \frac{1}{x+1} - \frac{2}{(2x-1)^{2}} \mathrm{d}x$$
